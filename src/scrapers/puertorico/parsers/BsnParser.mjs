/**
 * @file BsnParser.mjs
 * @description Zero-dependency JSON parser for FIBA LiveStats/Genius Sports Puerto Rico BSN box score pages.
 */
...
/**
 * @description Parses FIBA LiveStats JSON payload into Puerto Rico BoxScore Schema.
 ...
 */
export function parseBsnFibaJson(rawJson, gameId, season) {
	...
	if (!gameDate) {
		gameDate = `${season}-07-15`; // Default mid-summer date for BSN
	}
	...
	if (totalPlayersCount === 0) {
		throw new Error(`[FIBA JSON Error] Extracted 0 player records for BSN game ${gameId}`);
	}
...
export default { parseBsnFibaJson };
