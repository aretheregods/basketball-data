/**
 * @file AsiaParser.mjs
 * @description Robust zero-dependency regex-based HTML parser and FIBA LiveStats JSON parser for Asian box scores.
 */
...
/**
 * @description Parses FIBA LiveStats JSON payload into Asia BoxScore Schema.
 ...
 */
export function parseAsiaFibaJson(rawJson, gameId, season) {
...
/**
 * @description Parses raw Proballers HTML content into clean JSON matching Asia BoxScore Schema.
 ...
 * @returns {Object} Cleaned and structured box score object
 */
export function parseAsiaHtml(htmlContent, homeSlugExpected, awaySlugExpected, gameId, season) {
	...
	if (statsTables.length < 2) {
		throw new Error(`[DOM Error] Found fewer than 2 statistics tables for Asian game ${gameId}.`);
	}
