...
/**
 * @class AsiaScraper
 * @description Scraper and Orchestrator for Asian basketball competitions (EASL, WASL, BCL Asia, B.League, KBL, PBA, CBA, TPBL).
...
export class AsiaScraper extends HTTPClient {
	 ...
	 * @param {string|string[]} [options.competitions='easl'] - Comma-separated list or array of target competitions
	 * @param {string} [options.boxscoreType='traditional'] - Box score metric type
	 */
	constructor(options = {}) {
		super('https://www.proballers.com');

		// Parse competitions list (can be 'all', or comma-separated list like 'easl,wasl,bcl_asia,bleague,kbl,pba,cba,tpbl')
		const rawComps = options.competitions || 'easl';
		if (rawComps === 'all') {
			this.competitions = ['easl', 'wasl', 'bcl_asia', 'bleague', 'kbl', 'pba', 'cba', 'tpbl'];
	 ...
	 * Game ID is formatted as matchup-{COMP}{season}_{gameCode}, e.g. ryukyu-vs-seoul-sk-knights-EASL2025_10001.
	 * @param {string} gameId
	...
	parseGameId(gameId) {
		...
		const competitionId = segmentMatch ? segmentMatch[1].toLowerCase() : 'easl';
	...
	/**
	 * @description Returns the game ID itself, extracting the short ID segment if a full slug is passed.
	 * @param {string} gameId
	 * @returns {string}
	 */
	getGameUrl(gameId) {
	...
	getMockUnifiedBoxScore(gameId) {
					...
					players: [
						{
							...
							statistics: {
								...
								oreb: 5, dreb: 8, reb: 13, ast: 2, stl: 1, blk: 1, tov: 2, pf: 3, plus_minus: 6
							}
						}
					]
				},
					...
					players: [
						{
							...
							statistics: {
								min: "32:15", pts: 22, fgm: 8, fga: 18, fg3m: 4, fg3a: 10, ftm: 2, fta: 2,
								oreb: 0, dreb: 2, reb: 2, ast: 7, stl: 1, blk: 0, tov: 3, pf: 2, plus_minus: -6
							}
						}
					]
				}
		...
		if (competitionId === 'wasl') {
			return {
					...
					players: [
						{
							...
							statistics: {
								min: "30:15", pts: 24, fgm: 9, fga: 14, fg3m: 2, fg3a: 4, ftm: 4, fta: 5,
								oreb: 1, dreb: 3, reb: 4, ast: 8, stl: 2, blk: 0, tov: 1, pf: 1, plus_minus: 12
							}
						}
					]
				},
				awayTeam: {
					teamId: "GOR",
					teamName: "SHAHRDARI GORGAN",
					score: 84,
					players: [
						{
							playerId: "will-cherry",
							playerName: "Will Cherry",
							statistics: {
								min: "29:45", pts: 16, fgm: 6, fga: 13, fg3m: 2, fg3a: 6, ftm: 2, fta: 2,
								oreb: 1, dreb: 2, reb: 3, ast: 4, stl: 3, blk: 0, tov: 2, pf: 4, plus_minus: -12
							}
						}
					]
				}
				...
				players: [
					{
						...
						statistics: {
							min: "32:15", pts: 21, fgm: 8, fga: 16, fg3m: 0, fg3a: 1, ftm: 5, fta: 7,
							oreb: 3, dreb: 7, reb: 10, ast: 3, stl: 2, blk: 1, tov: 3, pf: 3, plus_minus: -6
						}
					}
				]
			}
