/**
 * @description Clock converter utility for NBA (12-minute quarters = 720 seconds; OT = 300 seconds).
 * Converts ISO 8601 duration string (e.g., "PT11M42.00S") or standard clock string ("11:42") into total seconds remaining in period.
...
export function parseClockToSeconds(clockStr, period) {
	...
	if (typeof clockStr === 'string' && clockStr.startsWith('PT')) {
		// ISO 8601 duration format e.g. PT11M42.00S or PT11M42S
...
/**
 * @description Calculates total game seconds remaining for NBA.
 * Regulation periods (1-4) are 12 minutes (720 seconds) each. Overtime (period 5+) is 5 minutes (300 seconds).
 * @param {number} period - Period / Quarter
 * @param {number} secondsInPeriod - Seconds remaining in current period
 * @returns {number} Total regulation/game seconds remaining
 */
export function calculateGameSecondsRemaining(period, secondsInPeriod) {
	...
	if (safePeriod <= 4) {
		return Number(((4 - safePeriod) * 720 + safeSecs).toFixed(2));
	}
...
function parseRawAction(gameId, action, index, headerMap = null) {
	...
	if (Array.isArray(action) && headerMap) {
		...
		actionNumber = action.actionNumber ?? action.actionId ?? action.eventNum ?? action.eventId ?? action.event_num ?? (index + 1);
		period = action.period ?? action.periodNumber ?? action.quarter ?? 1;

		const rawClock = action.clock ?? action.pcTimeString ?? action.timeRemaining ?? action.time_remaining ?? "12:00";
		...
		// Handle string actionType values
		...
		const rawSecondaryPlayerId = action.assistPersonId || action.blockPersonId || action.foulPersonId || action.foulDrawnPersonId || action.person2Id || action.player2Id || action.secondaryPlayerId || action.player2_id || null;
		secondaryPlayerId = (rawSecondaryPlayerId !== null && rawSecondaryPlayerId !== undefined) ? String(rawSecondaryPlayerId) : null;
	...
	const gameSecsRemaining = calculateGameSecondsRemaining(safePeriod, secsRemaining);
	...
	return {
		...
		game_seconds_remaining: isNaN(gameSecsRemaining) ? 0 : gameSecsRemaining,
...
/**
 * @description Lineup State Machine: processes actions chronologically to partition NBA game into 5-on-5 stints.
 * @param {string} gameId
...
function buildStintsFromEvents(gameId, events) {
	...
	for (const [period, pEvents] of periodMap.entries()) {
		...
		let stintStartClock = pEvents[0]?.clock || (period <= 4 ? "12:00" : "05:00");
		let stintStartSecs = pEvents[0]?.seconds_remaining ?? (period <= 4 ? 720 : 300);
...
/**
 * @description Main transformation function for NBA play-by-play payloads.
 * @param {string} gameId
