/**
 * @file SouthAmericaParser.mjs
 * @description Robust zero-dependency regex-based HTML parser and FIBA LiveStats JSON parser for South American box scores.
 */
...
/**
 * @description Parses FIBA LiveStats JSON payload into South America BoxScore Schema.
 ...
 */
export function parseSouthAmericaFibaJson(rawJson, gameId, season) {
	...
	if (totalPlayersCount === 0) {
		throw new Error(`[FIBA JSON Error] Extracted 0 player records for game ${gameId}`);
	}
...
/**
 * @description Parses raw Proballers HTML content into clean JSON matching South America BoxScore Schema.
 ...
 * @returns {Object} Cleaned and structured box score object
 */
export function parseSouthAmericaHtml(htmlContent, homeSlugExpected, awaySlugExpected, gameId, season) {
	...
	if (statsTables.length < 2) {
		throw new Error(`[DOM Error] Found fewer than 2 statistics tables for South America game ${gameId}.`);
	}
...
export default { parseSouthAmericaFibaJson, parseSouthAmericaHtml };
