...
| **nba** | `NBAScraper` | N/A | Pulls schedules from CDN endpoints, caching locally under `data/raw/nba/schedule_{year}.json`. Bypasses stats.nba.com to avoid Akamai blocks, extracting raw JSON state directly from `__NEXT_DATA__` on nba.com game pages. Supports Play-by-Play (`--type=pbp`) via `NbaPbpHarvester` (CDN live data endpoint, stats.nba.com fallback, Next.js page fallback) and `NbaPbpTransformer` (12-min quarter clock parsing, game_seconds_remaining calculation, and 5-on-5 stint tracking). |
